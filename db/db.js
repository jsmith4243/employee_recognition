...
db.run("CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT NOT NULL UNIQUE, password TEXT NOT NULL, salt TEXT NOT NULL, is_admin INTEGER, name TEXT, signature BLOB, created INTEGER)"); 
...
stmt.run("admin", hashPassword("password", salt), salt);
stmt.finalize();
